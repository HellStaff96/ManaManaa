...
# ПОЧЕМУ ОН РУГАЕТСЯ НА ЗВУК???

#ouch = mixer.Sound('Custom/Custom_sounds/ouch.wav')
...
mana = Dorime(0, -100, 25, 25, 20, power, "left")

def collides():
    global open_d, open_ch, coins_count
    key_pressed = key.get_pressed()
    for s in stairs_lst:
    ...
    for coin in coins:
        if sprite.collide_rect(hero, coin):
            ...
            coin.rect.y = -10000
            coins_count += 1
    #window.blit(transform.scale(image.load(coin_img)), (50,50)), (10, 10)
    coins_txt = font2.render('Mora: ' + str(coins_count), True, (214, 162, 245))
    window.blit(coins_txt, (70, 17)) 
    
    if key_pressed[K_SPACE]:
        mana.rect.y = hero.rect.centerx
        mana.rect.y = hero.rect.top
        items.add(mana)
        manas.add(mana)

    if sprite.spritecollide(Nechest1, manas, True):
        items.remove(mana)
        Nechest1.rect.x = -200
        items.remove(Nechest1)
    
    if sprite.spritecollide(Nechest2, manas, True):
        items.remove(mana)
        Nechest1.rect.x = -200
        items.remove(Nechest2)

    if sprite.spritecollide(Nechest3, manas, True):
        items.remove(mana)
        Nechest1.rect.x = -200
        items.remove(Nechest3)
    
    if sprite.collide_rect(hero, Nechest1):
        #ouch.play()
        hero.rect.x = 300
        hero.rect.y = 650
    
    if sprite.collide_rect(hero, Nechest2):
        pass
    
    if sprite.collide_rect(hero, Nechest3):
        pass
...
def start_pos():
    ...
    Nechest2 = Enemy(330, 480, 50, 50, 3, enemy_l, 'left')
    Nechest3 = Enemy(1900, 150, 50, 50, 3, enemy_l, 'left')
